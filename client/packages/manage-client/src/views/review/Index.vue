<template>
    <n-card class="card">
        <n-table striped>
            ...
            <tbody class="table-body">
                <tr :key="item.vid" v-for="item in videos">
                    <td>{{ item.vid }}</td>
                    <td>
    ...
    <n-drawer v-model:show="activeDrawer" :width="502">
        <n-drawer-content title="视频列表">
            <video-list :vid="vid" :list="resources"></video-list>
        </n-drawer-content>
    </n-drawer>
</template>
...
import { statusCode, reviewCode, getResourceUrl } from '@plooks/utils';
import type { VideoType, PartitionType } from '@plooks/apis';
import { getReviewListAPI, getResourceListAPI, reviewVideoAPI, getPartitionAPI } from '@plooks/apis';
