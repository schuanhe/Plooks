        ...
        <n-tabs default-value="account" size="large" justify-content="space-evenly" @update:value="loginTypeChange">
            <n-tab-pane class="form-container" name="account" :tab="t('login.accountLogin')">
                <n-form ref="accountFormRef" :rules="rules" :model="loginForm" label-placement="left" label-width="70">
                    <n-form-item :label="t('common.account')" path="user.username">
                        <n-input :placeholder="t('input.account')" v-model:value="loginForm.user.username" />
                    </n-form-item>
                    <n-form-item :label="t('common.pwd')" path="user.password">
                        <n-input :placeholder="t('input.pwd')" v-model:value="loginForm.user.password" type="password">
                            <template #suffix>
                                <n-button type="primary" text @click="findPassword">{{ t('login.forgotPwd') }}</n-button>
                            </template>
                        </n-input>
                    </n-form-item>
                    <n-form-item :label="t('common.code')" path="code" v-if="loginCaptcha.show">
            ...
            <n-tab-pane class="form-container" name="email" :tab="t('login.emailLogin')">
                <n-form ref="emailFormRef" :rules="rules" :model="loginForm" label-placement="left" label-width="70">
                    <n-form-item :label="t('common.email')" path="user.email">
                        <n-input :placeholder="t('input.email')" v-model:value="loginForm.user.email" />
                    </n-form-item>

                    <!-- 图像验证码 -->
                    <n-form-item :label="t('common.code')" v-show="!disabledSend">
                    ...
                    <n-form-item :label="t('common.ecode')" path="code">
                        <n-input :placeholder="t('input.ecode')" v-model:value="loginForm.code" />
