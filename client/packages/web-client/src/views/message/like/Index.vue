<template>
    <div class="msg-container" :style="themeStyle" @scroll="lazyLoading">
        <div class="msg-item" v-for="(item, index) in likeMessageList" :key="index">
            <div class="item-left">
...
import type { LikeMessageType } from '@plooks/apis';
import { getLikeMessageAPI } from '@plooks/apis';
...
let noMore = false;
let loading = false;
const likeMessageList = ref<Array<LikeMessageType>>([]);
const getLikeMessage = () => {
    loading = true;
    getLikeMessageAPI(page, pageSize).then((res) => {
        if (res.data.code === statusCode.OK) {
            if (res.data.data.messages.length < pageSize) {
                noMore = true;
            }
            likeMessageList.value = likeMessageList.value.concat(res.data.data.messages);
        }
        loading = false;
    }).catch(() => {
        loading = false;
