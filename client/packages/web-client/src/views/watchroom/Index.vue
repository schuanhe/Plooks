...
import { watchroomPlayer } from "@plooks/components";
import msg from './component/msg.vue';
import ArchiveInfo from './component/ArchiveInfo.vue';
import type Artplayer from 'artplayer';

import { Forbid } from "@plooks/icons";
import type { ResourceType, VideoType, roomInfoType,webSendMesgType,roomChatType } from '@plooks/apis';
import { getVideoInfoAPI,getRoomInfoApi, getAccessToken,updateRoomApi } from '@plooks/apis';
import { createUuid, globalConfig, statusCode, storageData, watchroomCode } from '@plooks/utils';
...
const msgDetails: Array<roomChatType> = [];//消息详情
...
//初始化weosocket
const initWebSocket = async(rid: number) => {
...
const websocketOnmessage = (e: any) => {
    // 将消息转换为json对象
    const data: webSendMesgType = JSON.parse(e.data);
    // 消息处理
    console.log(data);
...
const webSendMesg = (data:roomChatType) => {
     // 发送消息
     websocketSend({
